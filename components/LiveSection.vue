<template>
  <section class="a-live-section black position-relative">
    <div class="position-relative">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 950.36 334.45"
        style="transform: rotateZ(180)"
      >
        <defs>
          <radialGradient
            id="b"
            cx="475.49"
            cy="470.23"
            r="339.93"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#4216ff" />
            <stop offset=".13" stop-color="#4116fb" />
            <stop offset=".26" stop-color="#3e15ef" />
            <stop offset=".38" stop-color="#3913dc" />
            <stop offset=".5" stop-color="#3211c0" />
            <stop offset=".63" stop-color="#290e9d" />
            <stop offset=".75" stop-color="#1d0a71" />
            <stop offset=".87" stop-color="#10053d" />
            <stop offset=".99" stop-color="#010003" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="c"
            cx="473.65"
            cy="465.81"
            r="336.62"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#3e23fc" />
            <stop offset=".13" stop-color="#3d22f8" />
            <stop offset=".26" stop-color="#3a21ec" />
            <stop offset=".38" stop-color="#351ed9" />
            <stop offset=".51" stop-color="#2e1abd" />
            <stop offset=".63" stop-color="#261599" />
            <stop offset=".76" stop-color="#1b0f6e" />
            <stop offset=".88" stop-color="#0e083b" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="d"
            cx="471.82"
            cy="450.51"
            r="334.47"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#3a2ff9" />
            <stop offset=".13" stop-color="#392ef5" />
            <stop offset=".26" stop-color="#362ce9" />
            <stop offset=".38" stop-color="#3228d6" />
            <stop offset=".51" stop-color="#2b23ba" />
            <stop offset=".64" stop-color="#231c96" />
            <stop offset=".76" stop-color="#19146a" />
            <stop offset=".88" stop-color="#0d0a37" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="e"
            cx="469.99"
            cy="433.6"
            r="332.54"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#363cf6" />
            <stop offset=".13" stop-color="#353bf2" />
            <stop offset=".26" stop-color="#3338e6" />
            <stop offset=".39" stop-color="#2e33d2" />
            <stop offset=".51" stop-color="#282db7" />
            <stop offset=".64" stop-color="#202493" />
            <stop offset=".77" stop-color="#171967" />
            <stop offset=".89" stop-color="#0b0d34" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="f"
            cx="468.21"
            cy="416.81"
            r="330.69"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#3249f3" />
            <stop offset=".13" stop-color="#3148ef" />
            <stop offset=".26" stop-color="#2f44e3" />
            <stop offset=".39" stop-color="#2b3ecf" />
            <stop offset=".52" stop-color="#2536b3" />
            <stop offset=".64" stop-color="#1e2b8f" />
            <stop offset=".77" stop-color="#141e63" />
            <stop offset=".9" stop-color="#0a0f30" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="g"
            cx="466.55"
            cy="413.67"
            r="330.72"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#2e55f1" />
            <stop offset=".13" stop-color="#2d54ed" />
            <stop offset=".26" stop-color="#2b4fe1" />
            <stop offset=".39" stop-color="#2748cd" />
            <stop offset=".52" stop-color="#223fb1" />
            <stop offset=".65" stop-color="#1b328d" />
            <stop offset=".77" stop-color="#132261" />
            <stop offset=".9" stop-color="#09102e" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="h"
            cx="465"
            cy="411.4"
            r="331.47"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#2a62ee" />
            <stop offset=".13" stop-color="#2960ea" />
            <stop offset=".26" stop-color="#275bde" />
            <stop offset=".39" stop-color="#2453ca" />
            <stop offset=".52" stop-color="#1f48ae" />
            <stop offset=".65" stop-color="#18398a" />
            <stop offset=".78" stop-color="#11275e" />
            <stop offset=".91" stop-color="#07112a" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="i"
            cx="463.57"
            cy="409.13"
            r="332.85"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#266feb" />
            <stop offset=".13" stop-color="#256de7" />
            <stop offset=".26" stop-color="#2367db" />
            <stop offset=".39" stop-color="#205ec7" />
            <stop offset=".52" stop-color="#1c51ab" />
            <stop offset=".65" stop-color="#164087" />
            <stop offset=".79" stop-color="#0f2b5a" />
            <stop offset=".91" stop-color="#061227" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="j"
            cx="462.24"
            cy="406.86"
            r="334.83"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#227be8" />
            <stop offset=".13" stop-color="#2179e4" />
            <stop offset=".26" stop-color="#2073d8" />
            <stop offset=".39" stop-color="#1d68c4" />
            <stop offset=".53" stop-color="#1959a8" />
            <stop offset=".66" stop-color="#134683" />
            <stop offset=".79" stop-color="#0d2e57" />
            <stop offset=".92" stop-color="#051323" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="k"
            cx="461.01"
            cy="404.6"
            r="337.41"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#1e88e5" />
            <stop offset=".13" stop-color="#1d86e1" />
            <stop offset=".26" stop-color="#1c7ed5" />
            <stop offset=".4" stop-color="#1972c1" />
            <stop offset=".53" stop-color="#1662a5" />
            <stop offset=".66" stop-color="#114c80" />
            <stop offset=".8" stop-color="#0b3254" />
            <stop offset=".93" stop-color="#041320" />
            <stop offset="1" />
          </radialGradient>
          <clipPath id="a" transform="translate(0 -237.37)">
            <path fill="none" d="M100.33 0h750v500h-750z" />
          </clipPath>
        </defs>
        <g style="isolation: isolate">
          <g
            style="mix-blend-mode: screen"
            fill="none"
            stroke-miterlimit="10"
            stroke-width="2"
            opacity=".8"
            clip-path="url(#a)"
            data-name="Layer 2"
          >
            <path
              d="M949.68 421.73s-64.79-60.16-289.24 12.73-318.15 106.44-472 107.59S6.78 471.48 1 427.52"
              transform="translate(0 -237.37)"
              stroke="url(#b)"
            />
            <path
              d="M948.12 438.36s-66.42-72-292.11-8.59-315.55 102.75-468.34 97.79-177.5-77.27-181.5-121.15"
              transform="translate(0 -237.37)"
              stroke="url(#c)"
            />
            <path
              d="M946.56 455s-68-83.9-295-29.92-312.94 99.07-464.65 88-173.36-84-175.57-127.75"
              transform="translate(0 -237.37)"
              stroke="url(#d)"
            />
            <path
              d="M945 471.62s-69.68-95.77-297.86-51.24-310.34 95.39-461 78.18S17 407.9 16.53 364.21"
              transform="translate(0 -237.37)"
              stroke="url(#e)"
            />
            <path
              d="M943.45 488.25s-71.31-107.64-300.75-72.56-307.7 91.7-457.27 68.38S20.34 386.7 21.71 343.11"
              transform="translate(0 -237.37)"
              stroke="url(#f)"
            />
            <path
              d="M941.89 504.88S869 385.37 638.27 411s-305.13 88-453.58 58.57S23.74 365.51 26.9 322"
              transform="translate(0 -237.37)"
              stroke="url(#g)"
            />
            <path
              d="M940.33 521.51s-74.57-131.38-306.5-115.2-302.52 84.33-449.89 48.76S27.13 344.31 32.08 300.91"
              transform="translate(0 -237.37)"
              stroke="url(#h)"
            />
            <path
              d="M938.78 538.14S862.57 394.89 629.4 401.61s-299.92 80.66-446.21 39S30.52 323.12 37.26 279.81"
              transform="translate(0 -237.37)"
              stroke="url(#i)"
            />
            <path
              d="M937.22 554.76S859.38 399.65 625 396.92s-297.31 77-442.51 29.16S33.91 301.93 42.44 258.71"
              transform="translate(0 -237.37)"
              stroke="url(#j)"
            />
            <path
              d="M935.66 571.39s-79.46-167-315.13-179.16-294.71 73.29-438.83 19.35S37.3 280.73 47.62 237.61"
              transform="translate(0 -237.37)"
              stroke="url(#k)"
            />
          </g>
        </g>
      </svg>

      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 950.36 334.45">
        <defs>
          <radialGradient
            id="b"
            cx="475.49"
            cy="470.23"
            r="339.93"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#4216ff" />
            <stop offset=".13" stop-color="#4116fb" />
            <stop offset=".26" stop-color="#3e15ef" />
            <stop offset=".38" stop-color="#3913dc" />
            <stop offset=".5" stop-color="#3211c0" />
            <stop offset=".63" stop-color="#290e9d" />
            <stop offset=".75" stop-color="#1d0a71" />
            <stop offset=".87" stop-color="#10053d" />
            <stop offset=".99" stop-color="#010003" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="c"
            cx="473.65"
            cy="465.81"
            r="336.62"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#3e23fc" />
            <stop offset=".13" stop-color="#3d22f8" />
            <stop offset=".26" stop-color="#3a21ec" />
            <stop offset=".38" stop-color="#351ed9" />
            <stop offset=".51" stop-color="#2e1abd" />
            <stop offset=".63" stop-color="#261599" />
            <stop offset=".76" stop-color="#1b0f6e" />
            <stop offset=".88" stop-color="#0e083b" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="d"
            cx="471.82"
            cy="450.51"
            r="334.47"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#3a2ff9" />
            <stop offset=".13" stop-color="#392ef5" />
            <stop offset=".26" stop-color="#362ce9" />
            <stop offset=".38" stop-color="#3228d6" />
            <stop offset=".51" stop-color="#2b23ba" />
            <stop offset=".64" stop-color="#231c96" />
            <stop offset=".76" stop-color="#19146a" />
            <stop offset=".88" stop-color="#0d0a37" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="e"
            cx="469.99"
            cy="433.6"
            r="332.54"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#363cf6" />
            <stop offset=".13" stop-color="#353bf2" />
            <stop offset=".26" stop-color="#3338e6" />
            <stop offset=".39" stop-color="#2e33d2" />
            <stop offset=".51" stop-color="#282db7" />
            <stop offset=".64" stop-color="#202493" />
            <stop offset=".77" stop-color="#171967" />
            <stop offset=".89" stop-color="#0b0d34" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="f"
            cx="468.21"
            cy="416.81"
            r="330.69"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#3249f3" />
            <stop offset=".13" stop-color="#3148ef" />
            <stop offset=".26" stop-color="#2f44e3" />
            <stop offset=".39" stop-color="#2b3ecf" />
            <stop offset=".52" stop-color="#2536b3" />
            <stop offset=".64" stop-color="#1e2b8f" />
            <stop offset=".77" stop-color="#141e63" />
            <stop offset=".9" stop-color="#0a0f30" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="g"
            cx="466.55"
            cy="413.67"
            r="330.72"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#2e55f1" />
            <stop offset=".13" stop-color="#2d54ed" />
            <stop offset=".26" stop-color="#2b4fe1" />
            <stop offset=".39" stop-color="#2748cd" />
            <stop offset=".52" stop-color="#223fb1" />
            <stop offset=".65" stop-color="#1b328d" />
            <stop offset=".77" stop-color="#132261" />
            <stop offset=".9" stop-color="#09102e" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="h"
            cx="465"
            cy="411.4"
            r="331.47"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#2a62ee" />
            <stop offset=".13" stop-color="#2960ea" />
            <stop offset=".26" stop-color="#275bde" />
            <stop offset=".39" stop-color="#2453ca" />
            <stop offset=".52" stop-color="#1f48ae" />
            <stop offset=".65" stop-color="#18398a" />
            <stop offset=".78" stop-color="#11275e" />
            <stop offset=".91" stop-color="#07112a" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="i"
            cx="463.57"
            cy="409.13"
            r="332.85"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#266feb" />
            <stop offset=".13" stop-color="#256de7" />
            <stop offset=".26" stop-color="#2367db" />
            <stop offset=".39" stop-color="#205ec7" />
            <stop offset=".52" stop-color="#1c51ab" />
            <stop offset=".65" stop-color="#164087" />
            <stop offset=".79" stop-color="#0f2b5a" />
            <stop offset=".91" stop-color="#061227" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="j"
            cx="462.24"
            cy="406.86"
            r="334.83"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#227be8" />
            <stop offset=".13" stop-color="#2179e4" />
            <stop offset=".26" stop-color="#2073d8" />
            <stop offset=".39" stop-color="#1d68c4" />
            <stop offset=".53" stop-color="#1959a8" />
            <stop offset=".66" stop-color="#134683" />
            <stop offset=".79" stop-color="#0d2e57" />
            <stop offset=".92" stop-color="#051323" />
            <stop offset="1" />
          </radialGradient>
          <radialGradient
            id="k"
            cx="461.01"
            cy="404.6"
            r="337.41"
            gradientTransform="matrix(-1 0 0 1 950.67 0)"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stop-color="#1e88e5" />
            <stop offset=".13" stop-color="#1d86e1" />
            <stop offset=".26" stop-color="#1c7ed5" />
            <stop offset=".4" stop-color="#1972c1" />
            <stop offset=".53" stop-color="#1662a5" />
            <stop offset=".66" stop-color="#114c80" />
            <stop offset=".8" stop-color="#0b3254" />
            <stop offset=".93" stop-color="#041320" />
            <stop offset="1" />
          </radialGradient>
          <clipPath id="a" transform="translate(0 -237.37)">
            <path fill="none" d="M100.33 0h750v500h-750z" />
          </clipPath>
        </defs>
        <g style="isolation: isolate">
          <g
            style="mix-blend-mode: screen"
            fill="none"
            stroke-miterlimit="10"
            stroke-width="2"
            opacity=".8"
            clip-path="url(#a)"
            data-name="Layer 2"
          >
            <path
              d="M949.68 421.73s-64.79-60.16-289.24 12.73-318.15 106.44-472 107.59S6.78 471.48 1 427.52"
              transform="translate(0 -237.37)"
              stroke="url(#b)"
            />
            <path
              d="M948.12 438.36s-66.42-72-292.11-8.59-315.55 102.75-468.34 97.79-177.5-77.27-181.5-121.15"
              transform="translate(0 -237.37)"
              stroke="url(#c)"
            />
            <path
              d="M946.56 455s-68-83.9-295-29.92-312.94 99.07-464.65 88-173.36-84-175.57-127.75"
              transform="translate(0 -237.37)"
              stroke="url(#d)"
            />
            <path
              d="M945 471.62s-69.68-95.77-297.86-51.24-310.34 95.39-461 78.18S17 407.9 16.53 364.21"
              transform="translate(0 -237.37)"
              stroke="url(#e)"
            />
            <path
              d="M943.45 488.25s-71.31-107.64-300.75-72.56-307.7 91.7-457.27 68.38S20.34 386.7 21.71 343.11"
              transform="translate(0 -237.37)"
              stroke="url(#f)"
            />
            <path
              d="M941.89 504.88S869 385.37 638.27 411s-305.13 88-453.58 58.57S23.74 365.51 26.9 322"
              transform="translate(0 -237.37)"
              stroke="url(#g)"
            />
            <path
              d="M940.33 521.51s-74.57-131.38-306.5-115.2-302.52 84.33-449.89 48.76S27.13 344.31 32.08 300.91"
              transform="translate(0 -237.37)"
              stroke="url(#h)"
            />
            <path
              d="M938.78 538.14S862.57 394.89 629.4 401.61s-299.92 80.66-446.21 39S30.52 323.12 37.26 279.81"
              transform="translate(0 -237.37)"
              stroke="url(#i)"
            />
            <path
              d="M937.22 554.76S859.38 399.65 625 396.92s-297.31 77-442.51 29.16S33.91 301.93 42.44 258.71"
              transform="translate(0 -237.37)"
              stroke="url(#j)"
            />
            <path
              d="M935.66 571.39s-79.46-167-315.13-179.16-294.71 73.29-438.83 19.35S37.3 280.73 47.62 237.61"
              transform="translate(0 -237.37)"
              stroke="url(#k)"
            />
          </g>
        </g>
      </svg>
    </div>
  </section>
</template>

<script>
export default {
  name: 'LiveSection',

  components: {},

  data() {
    return {}
  }
}
</script>

<style lang="scss" scoped>
.a-live-section {
  // background: linear-gradient(5deg, black, #13196c, black);
  height: 500px;
  // animation: rotateGradientAngle 3s ease-in-out 40s infinite backwards;

  &::before {
    content: ' ';
    position: absolute;
    top: 0;
    left: 0;
    height: 100%;
    width: 100%;
    background: linear-gradient(-5deg, black, #13196c, black);
    animation: blink 8s ease-in-out infinite backwards;
    opacity: 0.8;

    // transform-origin: center;
    // animation: rotateAngle 3s ease-in-out 40s infinite backwards;
  }
}

@keyframes rotateGradientAngle {
  0% {
    background: linear-gradient(5deg, black, #13196c, black);
  }

  50% {
    background: linear-gradient(-5deg, black, #171e82, black);
  }
}

@keyframes blink {
  40% {
    opacity: initial;
  }
  50% {
    opacity: 1;
  }
  70% {
    opacity: initial;
  }
}

// @keyframes rotateAngle {
//   //
//   0% {
//   }
// }
</style>
